...
export function Modal({
  ...
  return (
    <Portal>
      <div
        className="fixed inset-0 z-50 grid place-items-center bg-[#0a0a0a]/45 p-3 backdrop-blur-sm sm:p-4"
        onClick={onClose}
      >
      <div
        role={role}
        ...
        className={cn(
          "flex max-h-[calc(100dvh-1.5rem)] w-full flex-col overflow-hidden rounded-xl sm:max-h-[calc(100dvh-2rem)]",
      ...
      >
        ...
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-border px-5 py-4 sm:px-6 sm:py-5">
          <div className="flex min-w-0 items-start gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <h2
                id={labelledBy}
                className={cn("font-display leading-tight", eyebrow ? "mt-1.5 text-xl sm:mt-2 sm:text-2xl" : "text-lg sm:text-xl")}
              >
                {title}
              </h2>
              {description && (
                <p className="mt-1.5 text-[0.8125rem] leading-snug text-muted-foreground sm:mt-2 sm:leading-relaxed">
                  {description}
                </p>
              )}
            </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">{children}</div>
          ...
          {/* ── Footer (pinned) ── */}
          {footer && (
            <div className="shrink-0 border-t border-border bg-muted/40 px-5 py-3.5 sm:px-6 sm:py-4">{footer}</div>
          )}
        </div>
      </div>
    </Portal>
